...
const PositionsTable = <T extends TPositionTableData, R extends TPositionTableOptions<T>>({
  ...
  arr.map((data: T) => {
    const values = keys.reduce(
      (acc: R[], key: keyof T) => {
        const deptData = {...(depts.find(item => item.id === data[DEPT_ID_KEY]) || { pagetitle: "", url: "" })};

        return [
          ...acc,
          {
            key,
            value: key === DEPT_ID_KEY ? deptData.pagetitle : String(data[key]),
            ...
            ...(depts.length > 0 && { [URL_KEY as string]: deptData.url })
          } as R
        ]
      },
      []
